...
for run_name in run_folders:
    ...
    # load image features based on the corresponding location embeddings
    if "resnet18_moco_20k" in run_name:

        image_csv = os.path.join(DATA_DIR, "resnet18_moco_20k.csv")
        if not os.path.exists(image_csv):
            raise FileNotFoundError(f"Coordinate CSV not found: {image_csv}")

        # read file and keep only image embedding columns
        image_pd_dataframe = pd.read_csv(image_csv)
        image_pd_cleaned = image_pd_dataframe.iloc[:,3:]

    elif "resnet_decur_20k" in run_name:

        image_csv = os.path.join(DATA_DIR, "resnet_decur_20k.csv")
        if not os.path.exists(image_csv):
            raise FileNotFoundError(f"Coordinate CSV not found: {image_csv}")
    ...
    elif "resnet_dino_20k" in run_name:

        image_csv = os.path.join(DATA_DIR, "resnet_dino_20k.csv")
        if not os.path.exists(image_csv):
            raise FileNotFoundError(f"Coordinate CSV not found: {image_csv}")
    ...
    elif "resnet_moco_20k" in run_name:

        image_csv = os.path.join(DATA_DIR, "resnet_moco_20k.csv")
        if not os.path.exists(image_csv):
            raise FileNotFoundError(f"Coordinate CSV not found: {image_csv}")
    ...
    elif "vit_dino_20k" in run_name:

        image_csv = os.path.join(DATA_DIR, "vit_dino_20k.csv")
        if not os.path.exists(image_csv):
            raise FileNotFoundError(f"Coordinate CSV not found: {image_csv}")
    ...
    elif "vit_moco_20k" in run_name:

        image_csv = os.path.join(DATA_DIR, "vit_moco_20k.csv")
        if not os.path.exists(image_csv):
            raise FileNotFoundError(f"Coordinate CSV not found: {image_csv}")
        ...
        print("No corresponding image embeddings found")
        continue
    ...
    # convert location embeddings to numpy
    loc_np = loc_features.cpu().numpy()

    # convert image embeddings (pandas df) to numpy
    image_np = image_pd_cleaned.to_numpy()

    # concatenate location + image embeddings (column-wise)
    concat_np = np.concatenate((loc_np, image_np), axis=1)

    # create column names
    loc_cols = [f"loc_dim_{i}" for i in range(loc_np.shape[1])]
    img_cols = [f"img_dim_{i}" for i in range(image_np.shape[1])]
    all_cols = [f"dim_{i}" for i in range(concat_np.shape[1])]

    # build final dataframe
    out_df = pd.DataFrame(concat_np, columns=all_cols)
    ...
    # reorder columns
    cols_order = (
        ["CODMUNI"] + all_cols + ["x", "y", "geometry"]
        if "CODMUNI" in out_df.columns
        else all_cols + ["x", "y", "geometry"]
    )
