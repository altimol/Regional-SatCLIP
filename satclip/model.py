...
from locationencoder.locationencoder import get_positional_encoding, get_neural_network, LocationEncoder
...
class SatCLIP(nn.Module):
    def __init__(self,
        ...
        # small learnable MLP head for fine-tuning
        ...
        self.location = LocationEncoder(
            positional_encoding_name=le_type,
            neural_network_name=pe_type,
            hparams=dict(
    ...
    def encode_image(self, image):
        # if there are already precomputed embeddings, just pass through the MLP
        if image.ndim == 2:
            return self.image_mlp(image)
        # otherwise, use the frozen CNN/ViT backbone
